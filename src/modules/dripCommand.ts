...
module.exports.cmd = async (client : Client, msg : Message, cmd : string) => {
    ...
    var filename = `${__dirname}\\..\\..\\generated\\drip_${mentions[0].id._serialized.split("@")[0]}.png`;
    try {
        if(fs.existsSync(filename)) { fs.unlinkSync(filename); }

        let template = await Canvas.loadImage(`${__dirname}\\..\\resources\\templates\\drip.jpg`);
        ...
        ctx.drawImage(pfpImg, 195, 70, 140, 140);
